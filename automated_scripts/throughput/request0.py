...
function_path = "/scripts/benchmarks/Sunspider/regexp-dna.js"
...
def post_request(args,master_num):
    post_response = requests.post(f"http://{ip[0]}:8080/run", data=json.dumps(run_data), headers=headers)
    return

# child container
req = int(sys.argv[1])
container_id = 7 + 0 * req
processes = []
