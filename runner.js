...
function initializeActionHandler(message) {
    ...
    } else if (!func_initialized) {
        let uselessWorker = new Worker('/scripts/useless.js');
         
        func_initialized = true;
   
        // code in worker thread's file
        let funcCode = `
            const outer_fs = require('fs');
            const {workerData, parentPort, threadId} = require('worker_threads');

            console.log("start require: " + threadId) 
            
            const paths = module.paths;
            const tempDir = "/tmp/node_modules";
            paths.unshift(tempDir);
            
            ${message.code}
            
            function responseMessage(code, response) {
                return {code: code, response: response, threadId: threadId};
            }
            
            var exec_start;
            var exec_end;
            outer_fs.openSync('/sharedVolume/ocall_stop', 'w+'); 

            new Promise(resolve => {
                resolve(main(workerData));
            }).then(result => {
                parentPort.postMessage(responseMessage(200, result));
                parentPort.close();
            });
        `;
